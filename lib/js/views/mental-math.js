...
export default class MentalMathView extends React.Component {
  ...
  view(opArgName) {
    if (this.isOperationArgumentAnswer(opArgName)) {
      return <input id={answer} type="text" className={styles.textRight} value={this.state.answer} onChange={this.handleChange.bind(this)} />;
    } else {
      return <input type="text" className={styles.textRight} value={this.model.toDisplay[opArgName]} disabled />;
    }
  }
