...
export default class MentalMath extends Game {
  ...
  chooseAnswer() {
    const n = randInt(1, 3);
    this.answer = n === 1 ? 'firstOperand' : (n === 2 ? 'secondOperand' : 'result');
  }
  ...
  generateWrongChoices() {
    ...
    for (let i = 1; i < choiceKeys.length; i++) {
  ...
  getChoices() {
    ...
    const n = randInt(0, choiceKeys.length - 1);
    const rightChoice = choiceKeys[n];
    ...
    for (let i = 0; i < choiceKeys.length; i++) {
      ...
      choices[choiceKeys[i]] = wrongChoices.pop();
    }
